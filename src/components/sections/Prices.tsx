    ...
    <section id="precios" className="bg-light py-16 sm:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal animation="fadeInUp" className="mx-auto mb-12 max-w-2xl text-center">
          ...
          <p className="mt-3 font-body text-foreground/70">{sections.prices.description}</p>
        </ScrollReveal>
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {items.map((p, i) => (
            ...
            >
              <h3 className="font-title text-xl text-headline">{p.title}</h3>
              <p className="mt-4 flex flex-wrap items-baseline gap-x-1.5 gap-y-0">
                {(p.from ?? true) && (
                  <span className="font-subtitle text-sm tracking-wide text-primary/70 uppercase">
                  Desde
                </span>
                )}
                <span className="font-title text-4xl text-primary">${p.price}</span>
                <span className="text-sm text-foreground/50">{p.currency}</span>
              </p>
              <p className="mt-3 flex items-center gap-2 font-body text-sm text-foreground/60">
                <Clock className="h-4 w-4 text-primary" />
                Duración: {p.duration}
              </p>
              <ul className="mt-5 flex-1 space-y-2">
                {p.notes.map((n) => (
                  <li key={n} className="flex items-start gap-2 font-body text-sm text-foreground/70">
                    <Check className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />
                    {n}
                  </li>
                ))}
              </ul>
              <BookCta className="mt-6 inline-flex w-full items-center justify-center rounded-full bg-primary px-5 py-3 font-subtitle text-sm text-white transition-all hover:bg-primary-dark">
                Agenda tu cita aquí
              </BookCta>
            </motion.article>
          ))}
